...
| Input                  | Unity's New Input System (not legacy Input Manager)                                       | Actively developed; `PlayerInput` component's device-pairing is built for local co-op (multiple players, multiple devices, one machine). Uses a custom `PlayerControls` Input Actions asset (not Unity's auto-generated default) for full control over action names. |
