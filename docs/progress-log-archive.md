# Progress Log Archive — Sessions 1-27

Sessions 1-9 (pre-boss, single-player fundamentals) and Sessions 10-27 (the
boss-encounter arc through the Level 1 rework) archived out of
`progress-log.md` for length, in two passes. This is a pure relocation, not a
rewrite — content below is unchanged from when it lived in `progress-log.md`,
including any historical inaccuracies later sessions deliberately left
alone (see the Session 18 "SampleScene" addendum policy in
[progress-log.md](progress-log.md)). Continues in
[progress-log.md](progress-log.md) at Session 28.
...
## Session 27 — Level 1 Rework: Sequencing + Scripted Boss Movement

User request: stop treating the boss fight as an instant, permanent
encounter and frame it as "Level 1" — one of many levels to come, with a
proper opening (ships enter, free movement, minions fight for a while, then
the boss makes an entrance) instead of dropping the party straight into
combat, and a boss that moves on an authored pattern rather than AI.

### Design decisions, confirmed upfront

- **Minion source for the pre-boss/phase-2 phases**: reuse
  `EnemySpawner.cs`'s existing wave-formation system (`Random`/`Line`/
  `Cluster`/`VFormation`), not the boss-flanking `MinionSpawner`/`Minion`
  system — the latter anchors to a live boss transform, which doesn't exist
  yet during the pre-boss phase. `MinionSpawner`/`Minion` stay attached to
  the boss and functional, just unwired from the Level 1 sequence for now.
- **Rename scope**: only the boss itself becomes `Level1Boss` (class,
  script file, prefab) since future levels will bring their own bosses.
  `Gameplay.unity` keeps its name — it's meant to be reused by future
  levels, not renamed per-level. The in-scene GameObject stays named `Boss`
  (only the script/prefab asset names changed); every other script keeps
  its generic name (`BossPanelUI`, `EnemySpawner`, `Enemy`, ...).
- **Boss entrance duration**: 4 seconds, matching the ship intro's pacing.
- Movement pattern loops unchanged through both phases (no phase-2
  variant), "snap to a side" is a literal one-frame teleport not a fast
  ease, and the advance/retreat/return-home legs share one
  `patternMoveDuration` tunable rather than three separate ones — all
  simplest-reading defaults, none specified explicitly by the user.

### Rename: `Boss` → `Level1Boss`

Renamed `Boss.cs`/`Boss.prefab` to `Level1Boss.cs`/`Level1Boss.prefab` via
the Unity MCP bridge's asset rename (preserves `.meta` GUIDs — confirmed
afterward that every existing Inspector reference, including all 4
`PlayerController.boss` fields, `BossPanelUI.boss`, and `Level1Boss`'s own
`targets[]` array, survived intact with zero manual re-wiring). Updated
every remaining `Boss` type reference for the rename to compile:
`PlayerController.cs`, `AIController.cs`, `Bullet.cs`, `BossPanelUI.cs`,
`MinionSpawner.cs`, `Minion.cs` (the last two are compile-fixes only —
still unused in the active fight sequence).

### `LevelSequencer.cs` (new)

A single new orchestrator MonoBehaviour, owning the whole pre-fight
timeline as one linear coroutine over 6 states (`Intro` →
`FreeMovement` → `MinionPhase1` → `WaitingForClear` → `BossEntrance` →
`BossCombat`). This is a deliberate, acknowledged exception to
`architecture.md`'s "no manager/singleton" rule — the feature is inherently
a top-level sequence. Named generically (not `Level1`-prefixed) since the
scene/script are meant to be reused by future levels.

Ships/boss "off-screen" start positions are computed at runtime from
`Camera.main.ViewportToWorldPoint`, not authored marker Transforms — same
idiom `Level1Boss.ClampToBounds()`/`PlayerController.HandleMovement()`
already use for viewport clamping. Each ship's "home" is just wherever it's
already scene-placed, captured once in `Awake()`; the boss's home is
whatever position it's authored at.

**Freeze mechanics gotcha, found before it caused a live bug**: disabling
only `PlayerInput`/`AIController` during the intro/boss-entrance freeze
windows is not enough — `PlayerController.HandleMovement()` runs every
`FixedUpdate` and its held-fire check runs every `Update()` regardless of
those driver components' enabled state, so a ship mid-input the instant a
freeze starts would keep drifting/firing. `SetShipsFrozen()`'s correct
order: disable `PlayerInput`/`AIController` first, clear stale input via
`SetMoveDirection(Vector2.zero)`/`SetFiring(false)`, then disable
`PlayerController` itself last (unfreeze reverses the order).
`PlayerAbility`'s ability input already flows entirely through those same
two driver paths, so no separate lock was needed to block ability use.

Phase-2 minions needed **zero** sequencer state: `Level1Boss.OnPhase2` got
a new persistent `UnityEvent` listener straight to
`EnemySpawner.StartSpawning()`, same decoupled-notification shape as
`OnDefeated`'s existing listeners.

### `Level1Boss.cs` — scripted movement pattern

Replaced `HandleMovement()`'s random dash-or-hold entirely with a looping
coroutine (`OnEnable()` → `MovementPatternRoutine()` → `RunCycle()` →
`MoveOverTime()`): snap instantly to one side, advance toward the ships,
retreat, return home, wait `cycleGap` (1.5s), repeat mirrored to the other
side, forever. `OnEnable()` (not `Awake()`) captures `home`, since it needs
to fire fresh every time `LevelSequencer` enables the component after the
entrance glide, not once at scene load. `ClampToBounds()` — the viewport/
`maxAdvanceFraction` clamp — is reused as-is from the old system, so the
descent limit is governed by the same tuning knob it always was. The
`enemySpawner` field/`Awake()`-time disable of `Spawner` was removed from
`Level1Boss` entirely; `LevelSequencer` is now the sole owner of when
`Spawner` runs.

### `EnemySpawner.cs` / `Enemy.cs`

`EnemySpawner` no longer self-starts (`Start()`'s `InvokeRepeating` was
removed); new `public StartSpawning()`/`StopSpawning()` are called
externally by `LevelSequencer`. Formation selection changed from a fixed
escalating cycle (`formationOrder[waveIndex % formationOrder.Length]`) to a
plain random pick (`Random.Range`) — no no-repeat guard, since this is the
only caller of `formationOrder` today and the user asked for "random order,
for now."

`Enemy.cs` gained a `public static readonly List<Enemy> Active` registry,
populated/depopulated in new `Awake()`/`OnDestroy()` methods, mirroring
`Minion.Active`/`Bullet.Active` exactly — this is what lets
`LevelSequencer` detect "zero enemies on screen" before starting the boss's
entrance.

### Scene wiring (`Gameplay.unity`)

New `LevelSequencer` GameObject, wired to all 4 ships/`Spawner`/the boss
instance. `Level1Boss`'s component set to disabled by default (`enabled:
false`) so it's inert until `LevelSequencer` explicitly enables it after
the entrance. Repositioned the 4 ships' Transform positions into an actual
horizontal line near the bottom of the screen (`y = -4.2`, evenly spaced in
`x`) — they'd previously been scattered (e.g. `Player` at the origin,
`Teammate_Tank` off to one side at a different height), since nothing
before this needed them to form a line.

### Verified

Unity MCP bridge, Play mode. **Found and fixed a real environment quirk
first**: with the Unity Editor window unfocused (this MCP session drives
Unity with no human at the keyboard), `Time.time`/`Time.frameCount` simply
don't advance at all — confirmed via `execute_code` (`Time.time` stuck at
`0.04` after 5+ real seconds). Setting `Application.runInBackground = true`
at runtime fixed it immediately. Not a bug in this session's code, but
worth remembering for any future headless Play-mode verification in this
project.

With that fixed (and `minionPhase1Duration` temporarily lowered to 8s for
faster iteration, restored to 120 afterward), confirmed via
`LevelSequencer.CurrentState`/`execute_code` snapshots at each transition:
intro correctly starts ships off-screen-below and frozen
(`PlayerController.enabled == false`); handoff at 4s unfreezes them; minion
waves start at 8s and formation order is visibly non-escalating; waves stop
and the boss entrance waits for `Enemy.Active.Count == 0` before moving;
the boss glides in from off-screen-top and `ClampToBounds()` correctly caps
both X and Y during the movement pattern (`bossPos` landed exactly at the
clamped floor/side-offset in separate checks); forcing `TakeDamage()` past
the 50%-HP line flipped `IsPhase2` and the `OnPhase2` listener correctly
resumed `EnemySpawner` (10 new enemies appeared); a final forced
`TakeDamage()` past 0 destroyed the boss cleanly (`OnDefeated` fired, no
console errors throughout either Play session).

**Not a bug, a balance observation**: the party wiped out during
unattended automated combat both times this was tested end-to-end — the
human `Player` ship receives no real input in this headless environment
(nobody pressing WASD/Fire), and the new movement pattern is deterministic
and consistently aggressive (advances toward the ships every ~3.6s cycle)
compared to the old dash's 35%-probability-per-1.5s randomness. Worth a
real human playtest to judge actual difficulty; not something adjusted in
this session since it wasn't part of the ask.

### Still open

- Real human playtest of the new pacing/difficulty (see balance
  observation above) — this session only verified the sequencing/mechanics
  are correct, not that the resulting fight feels good.
- `MinionSpawner`/`Minion` (boss-flanking adds) remain attached to the boss
  but unused by Level 1 — a future level or a later pass could reactivate
  them, or they could be removed if they end up staying dead code.
- Local co-op / dynamic player count, scene scaffolding, and Nakama
  networking — unchanged, still the next roadmap milestones.
